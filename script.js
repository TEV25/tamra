// ===================================================================
// =============== إعدادات عامة ومتغيرات الموقع ====================
// ===================================================================

// رقم الواتساب الخاص بالمتجر (لا تغيره إلا إذا أردت تغيير رقم التواصل)
const WHATSAPP_NUMBER = "0539836477";

// سلة المشتريات: يتم تخزينها في localStorage عشان تفضل محفوظة بعد إعادة تحميل الصفحة
...
document.addEventListener("DOMContentLoaded", function() {
    ...
    // استرجاع الثيم المحفوظ (الوضع الفاتح/الداكن)
    ...
    // إضافة إمكانية إغلاق السلة بالسحب (سحب لليسار) على الأجهزة الجوالة
...
// ===================================================================
// =============== طريقة إضافة منتج جديد مستقبلًا ====================
// ===================================================================
/*
🔰 طريقة إضافة منتج جديد:

1. انتقل إلى القسم المناسب (سخانات / صحون / مناسبات / مشروبات)
2. أضف كائنًا جديدًا داخل المصفوفة مع الانتباه إلى:
   - يجب أن يكون id فريدًا وغير مكرر
   - انتبه للفواصل بين المنتجات
   - تأكد من اسم الصورة ووجودها داخل مجلد images

📌 مثال لإضافة أكلة جديدة في قسم السخانات:
{
    id: 30,                               // رقم تسلسلي جديد (لا يتكرر مع أي منتج آخر)
    name: "اسم الأكلة الجديدة",           // اسم المنتج (سيظهر للعميل)
    type: "sakhanat",                     // مهم جدًا: يحدد القسم الذي سيظهر فيه المنتج
    category: "سخانات",                  // التصنيف الفرعي
    mainImage: "images/اسم الصورة.webp", // الصورة الرئيسية
    images: ["images/اسم الصورة.webp"],  // معرض الصور (يمكن إضافة عدة صور)
    description: "وصف تفصيلي للأكلة"     // يظهر داخل نافذة التفاصيل
}

📌 مثال لإضافة مشروب بارد جديد:
{
    id: 428,
    name: "عصير مانجو",
    type: "beverages",
...
// ===================================================================
// =============== أكثر الأخطاء الشائعة عند الإضافة ==================
// ===================================================================
/*
⚠️ أخطاء شائعة تجنبها:
1. نسيان الفاصلة (,) بين المنتجات داخل المصفوفة.
2. تكرار نفس id مع منتج آخر.
3. كتابة type خاطئ (يجب أن يكون مطابقًا لأحد الأنواع الموجودة).
4. عدم وضع الصورة داخل مجلد images بنفس الاسم المذكور.
5. نسيان إضافة description أو جعلها نصًا فارغًا (يظهر للمستخدم "لا يوجد وصف").
6. تغيير أسماء الدوال أو المتغيرات الأساسية.
*/

// ===================================================================
// =============== منتجات السخانات (الأكلات الساخنة) =================
// ===================================================================
...
const eventsProducts = [
    { id: 316, name: "مباشرين ومباشرات", type: "events", category: "تجهيزات المناسبات", mainImage: "images/مباشرين ومباشرات.webp", images: [ "images/مباشرين ومباشرات (1).webp", "images/مباشرين ومباشرات (2).webp", "images/مباشرين ومباشرات (3).webp", "images/مباشرين ومباشرات (4).webp" ], description: "" },
...
// دالة الاستفسار عن طلب غير موجود (ما حصلت طلبك؟)
function sendInquiry() {
    const inquiryMessage = `السلام عليكم،
حاب أستفسر عن خدمة أو طلب غير موجود بالموقع.`;
    window.open(`https://wa.me/966539836477?text=${encodeURIComponent(inquiryMessage)}`, "_blank");
}
...
// ===================================================================
// =============== تأثير الظهور السلس عند التمرير (Reveal) ===========
// ===================================================================
...
// 3. تأثير تموج للأزرار
function initRippleEffect() {
    const buttons = document.querySelectorAll('.btn-primary, .btn-secondary, .btn-add, .btn-details, .btn-add-package, .checkout-btn');
    buttons.forEach(btn => {
        btn.classList.add('ripple-btn');
    });
}

// 4. شريط منبثق للخصومات
function showPromoPopup() {
    ...
    if (!hasSeen) {
        setTimeout(() => {
            ...
            popup.innerHTML = '<i class="fas fa-gift"></i><span>خصم 10% على أول طلب! استخدم كود: WELCOME10</span>';
            popup.onclick = () => {
                const searchInput = document.getElementById('searchInput');
                if (searchInput) searchInput.value = 'خصم 10%';
                popup.remove();
            };
...
// 6. عدادات متحركة للأرقام
function initScrollCounters() {
    const counters = document.querySelectorAll('.stat span, .hero-stats .stat span');
    const observer = new IntersectionObserver((entries) => {
        entries.forEach(entry => {
            if (entry.isIntersecting) {
                const counter = entry.target;
                const target = parseInt(counter.innerText);
                if (!isNaN(target) && target > 0 && !counter.hasAttribute('data-counted')) {
...
function initScrollReveal() {
    const elements = document.querySelectorAll('.product-card, .category-card, .feature-item, .contact-card, .package-card-modern');
    const observer = new IntersectionObserver((entries) => {
        entries.forEach(entry => {
            if (entry.isIntersecting) {
...
// 8. تحسين الصور المتحركة (وقف الحركة عند عدم الرؤية)
...
// 9. تحميل الصور بشكل تدريجي (lazy loading محسن)
...
// 10. منع التمرير الخلفي عند فتح السلة
...
// 12. إظهار آخر منتج تم إضافته للسلة
function showLastAdded() {
    const lastAdded = localStorage.getItem('lastAdded');
    if (lastAdded) {
        setTimeout(() => {
            showToast(`🛒 آخر منتج تمت إضافته: ${lastAdded}`, 'info');
...
// تحديث دالة addToCart لتخزين آخر منتج
const originalAddToCart = addToCart;
window.addToCart = function(id) {
    const product = allProducts.find(p => p.id === id);
    if (product) {
        localStorage.setItem('lastAdded', product.name);
    }
